...
export async function POST(req) {
  ...
  if (secret !== process.env.WEBHOOK_SECRET) {
    logger.error('Invalid webhook secret', { ip, receivedSecret: secret ? '***' : 'missing' });
  ...
  try {
    ...
    try {
      ...
      logger.error('Invalid JSON payload', { ip, body: body.substring(0, 200) });
...
import { processNotionWebhook, processGithubWebhook, processCustomWebhook } from './integrations';
...
export function createWorker() {
  return new Worker('webhook-queue', async (job) => {
...
async function ErrorList() {
  ...
  const errors = [
    { id: 1, time: '2023-10-15 14:30', message: 'GitHub API rate limit exceeded', provider: 'github' },
    { id: 2, time: '2023-10-15 12:15', message: 'Invalid Notion database ID', provider: 'notion' }
  ];
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {errors.map((error) => (
            <tr key={error.id}>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{error.time}</td>
